    ...
    for id in rp2:
        if id['primary_ip4'] != None:
            ...
            for index, row in df.iterrows():
                sql = "INSERT INTO Device_test_2 (Instance_ID, Instance_name, Device_ID, Parent_instance, Manager_name, Manager_mail, Manager_tenant, Type, Status, Level_importance, Monitored, Primary_ip4, Primary_ip6) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Device_ID=%s, Parent_instance=%s, Manager_name=%s, Manager_mail=%s, Manager_tenant=%s, Type=%s, Status=%s, Level_importance=%s, Monitored=%s, Primary_ip6=%s"
                val = (row['instance_id'], row['Name'], row['Device_id'], row['Parent_instance'], row['Manager_first_name'], row['Manager_mail'], row['tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['primary_ip4'], row['primary_ip6'], row['Device_id'], row['Parent_instance'], row['Manager_first_name'], row['Manager_mail'], row['tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['primary_ip6'])
