...
for ip in ipfile:
    qr_ip = {"address":ip}
    rp= requests.request("GET", ip_url, headers=headers, params=qr_ip).json()['results'][0]  # chuyen response ve json roi "di sau vao gia tri dau tien cua mang list result"?
    ...
    if as_name != None:
        ...
        hba_p1=str(rp_hba['metadata_info']['HBA']['rows'][0]['port_name']) # day la chuyen json thanh str?
        hba_p2=str(rp_hba['metadata_info']['HBA']['rows'][1]['port_name'])
    else:
        hba_p1='blank'
        hba_p1='blank'
    
    print (ip.strip() + ' ' + hba_p1 + ' ' + hba_p2 ) # ip.strip la gi
