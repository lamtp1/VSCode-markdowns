...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO LB_pool_member (LB_pool_member_ID, LB_pool_ID, Primary_ip4, Name, Status) VALUES (%s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE   Primary_ip4=%s, Name=%s, Status=%s"
            val = (row['LB_pool_member_ID'], row['LB_pool_ID'], row['Primary_ip4'], row['Name'], row['Status'], row['Primary_ip4'], row['Name'], row['Status'])
