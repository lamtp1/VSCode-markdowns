...
ha_qr= 'hypervisor_vcpus_used{aggregate=~".*"}'
payload = {'query': ha_qr}
...
for i in rq:
    if str(i["metric"]["monitor"]) != "openstack-hlct5":
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO victoria_compute_used_cpu (Hostname_compute, ha_name, IP_Compute, Cum_Cloud, vCPU, insert_time) VALUES (%s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Hostname_compute=%s, IP_Compute=%s, Cum_Cloud=%s, vCPU=%s, insert_time=%s"
            val = (row['Hostname_compute'], row['ha_name'], row['IP_Compute'], row['Cum_Cloud'], row['vCPU'], insert_time, row['Hostname_compute'], row['IP_Compute'], row['Cum_Cloud'], row['vCPU'], insert_time)
