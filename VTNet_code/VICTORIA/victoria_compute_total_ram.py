...
ha_qr= 'hypervisor_memory_mbs_total{aggregate=~".*"}/1024'
payload = {'query': ha_qr}
...
for i in rq:
    if str(i["metric"]["monitor"]) != "openstack-hlct5":
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO victoria_compute_total_ram (Hostname_compute, ha_name, IP_Compute, Cum_Cloud, RAM, insert_time) VALUES (%s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Hostname_compute=%s, IP_Compute=%s, Cum_Cloud=%s, RAM=%s, insert_time=%s"
