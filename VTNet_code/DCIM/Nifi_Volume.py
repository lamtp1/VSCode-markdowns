...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO Volume (Volume_ID, Pool_ID, Pool_name, insert_time) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Pool_ID=%s, Pool_name=%s"
            val = (row['Volume_ID'], row['Pool_ID'], row['Pool_name'], insert_time, row['Pool_ID'], row['Pool_name'])
