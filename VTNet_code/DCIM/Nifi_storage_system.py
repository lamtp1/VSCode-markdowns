...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO Storage_System (Storage_System_ID, Name, Manufacturer, Type, Tenant, Manager_name, Manager_email, Total_space, Used_space, Free_space, insert_time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Name=%s, Manufacturer=%s, Type=%s, Tenant=%s, Manager_name=%s, Manager_email=%s, Total_space=%s, Used_space=%s, Free_space=%s"
            val = (row['Storage_System_ID'], row['Name'], row['Manufacturer'], row['Type'], row['Tenant'],  row['Manager_name'], row['Manager_email'], row['Total_space'], row['Used_space'], row['Free_space'], insert_time, row['Name'], row['Manufacturer'], row['Type'], row['Tenant'],  row['Manager_name'], row['Manager_email'], row['Total_space'], row['Used_space'], row['Free_space'])
