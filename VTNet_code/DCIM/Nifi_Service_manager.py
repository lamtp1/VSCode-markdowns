...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO Service_Management (Service_ID, Manager_ID, Manager_name, Manager_email, insert_time) VALUES (%s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Manager_ID=%s, Manager_name=%s, Manager_email=%s"
            val = (row['Service_ID'], row['Manager_ID'], row['Manager_name'], row['Manager_email'], insert_time, row['Manager_ID'], row['Manager_name'], row['Manager_email'])
