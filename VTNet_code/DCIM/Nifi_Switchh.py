...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        s = Switch_ID + "|" + Name + "|" + Group_name + "|" + Vlan_group + "|" + Platform + "|" + Type + "|" + Layer + "|" + Firmware_version + "|" + Manager_name + "|" + Manager_email + "|" + Verify_status + "\n"
        switch_file.write(s)
        ...
        #     sql = "INSERT INTO Switch (Switch_ID, Group_name, Vlan_group, Platform, Type, Layer, Firmware_version, Manager_name, Manager_email, Verify_status, insert_time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Group_name=%s, Vlan_group=%s, Platform=%s, Type=%s, Layer=%s, Firmware_version=%s, Manager_name=%s, Manager_email=%s, Verify_status=%s"
        #     val = (row['Switch_ID'], row['Group_name'], row['Vlan_group'], row['Platform'], row['Type'], row['Layer'], row['Firmware_version'], row['Manager_name'], row['Manager_email'], row['Verify_status'], insert_time, row['Group_name'], row['Vlan_group'], row['Platform'], row['Type'], row['Layer'], row['Firmware_version'], row['Manager_name'], row['Manager_email'], row['Verify_status'])
