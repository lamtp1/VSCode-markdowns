...
while offset <  250:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO LB_pool (LB_pool_ID, Code, Vip_Endpoint_ID, Algorithm, Protocol, Status, insert_time) VALUES (%s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Code=%s, Vip_Endpoint_ID=%s, Algorithm=%s, Protocol=%s, Status=%s"
            val = (row['LB_pool_ID'], row['Code'], row['Vip_Endpoint_ID'], row['Algorithm'], row['Protocol'], row['Status'], insert_time, row['Code'], row['Vip_Endpoint_ID'], row['Algorithm'], row['Protocol'], row['Status'])
