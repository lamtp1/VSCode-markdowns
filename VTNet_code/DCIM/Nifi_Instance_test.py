...
limit= 250
offset= 0
instance_file = open('instance_new.txt', 'w', encoding = 'UTF-8')
header_hafile="Instance_ID|Instance_os|Instance_name|Device_ID|Parent_instance|Manager_name|Manager_tenant|Type|Status|Level_importance|Monitored|Primary_ip4|created|cloud|insert_time \n"
instance_file.write(header_hafile)
while offset <  ip_count:
    querystring = {"limit":limit, "offset": offset}
    rp2= requests.request("GET",instance_url, headers=headers, params=querystring, timeout=1200).json()['results']
    for id in rp2:
        if id['primary_ip4'] != None:
            ...
            s = str(instance_id) + '|' + os + '|' + instance_name + '|' + str(device_id) + '|' + parent_instance + '|' + owner_name + '|' + tenant + '|' + Type + '|' + status + '|' + level_important + '|' + monitored + '|' + primary_ip4 + '|' + created + '|' + cloud + '|' + insert_time + '|' + '\n'
            ...
            if id['os'] != None:
                ...
                os = 'null'
            r = str(instance_id) + '|' + os+ '\n'
            instance_file.write(r)
