    ...
    for id in rp2:
        if id['primary_ip4'] != None:
            ...
            for index, row in df.iterrows():
                sql = "INSERT INTO Instance (Instance_ID, Instance_name, Device_ID, Parent_instance, Manager_name, Manager_mail, Manager_tenant, Type, Status, Level_importance, Monitored, Primary_ip4, Primary_ip6, created,  insert_time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Device_ID=%s, Parent_instance=%s, Manager_name=%s, Manager_mail=%s, Manager_tenant=%s, Type=%s, Status=%s, Level_importance=%s, Monitored=%s, Primary_ip6=%s, created=%s"
                val = (row['Instance_ID'], row['Instance_name'], row['Device_ID'], row['Parent_instance'], row['Manager_name'], row['Manager_mail'], row['Manager_tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['Primary_ip4'], row['Primary_ip6'], row['created'], insert_time, row['Device_ID'], row['Parent_instance'], row['Manager_name'], row['Manager_mail'], row['Manager_tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['Primary_ip6'], row['created'])
