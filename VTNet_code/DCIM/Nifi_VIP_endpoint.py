...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO VIP_Endpoint (VIP_Endpoint_ID, Code, Name, LB_ID, VIP_address, Port, Protocol, Status, insert_time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Code=%s, Name=%s, LB_ID=%s, VIP_address=%s, Port=%s, Protocol=%s, Status=%s"
            val = (row['VIP_Endpoint_ID'], row['Code'], row['Name'], row['LB_ID'], row['VIP_address'], row['Port'], row['Protocol'], row['Status'], insert_time, row['Code'], row['Name'], row['LB_ID'], row['VIP_address'], row['Port'], row['Protocol'], row['Status'])
