...
while offset <  ip_count:
    ...
    for id in rp2:
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO Service (Service_ID, Code, Service_name, Service_group, Status, Tenant, insert_time) VALUES (%s, %s, %s, %s, %s,%s, %s) ON DUPLICATE KEY UPDATE  Code=%s, Service_name=%s, Service_group=%s, Status=%s, Tenant=%s"
            val = (row['Service_ID'], row['Code'], row['Service_name'], row['Service_group'], row['Status'], row['Tenant'], insert_time, row['Code'], row['Service_name'], row['Service_group'], row['Status'], row['Tenant'])
