...
for i in rq:
    if str(i["metric"]["monitor"]) != "openstack-hlct5":
        ...
        for index, row in df.iterrows():
            sql = "INSERT INTO Cloud_HA (cloud_cluster, ha_name, compute_hm, compute_ip, nova_status) VALUES (%s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  cloud_cluster=%s, ha_name=%s, compute_hm=%s, compute_ip=%s, nova_status=%s"
            val = (row['cloud_cluster'], row['ha_name'], row['compute_hm'], row['compute_ip'], row['nova_status'], row['cloud_cluster'], row['ha_name'], row['compute_hm'], row['compute_ip'], row['nova_status'])
