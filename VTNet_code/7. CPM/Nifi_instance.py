...
limit= 250
offset= 0
Nifi_Instance_file = open("Nifi_Instance_file.txt", "w+", encoding='UTF-8')
while offset <  ip_count:
    ...
    for id in rp2:
        if id['primary_ip4'] != None:
            ...
            s = str(instance_id) + "|" + instance_name + "|" + primary_ip4 + "\n"
            Nifi_Instance_file.write(s)
            ...
            #     sql = "INSERT INTO Instance (Instance_ID, Instance_name, Device_ID, Parent_instance, Manager_name, Manager_mail, Manager_tenant, Type, Status, Level_importance, Monitored, Primary_ip4, Primary_ip6, created) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Device_ID=%s, Parent_instance=%s, Manager_name=%s, Manager_mail=%s, Manager_tenant=%s, Type=%s, Status=%s, Level_importance=%s, Monitored=%s, Primary_ip6=%s, created=%s"
            #     val = (row['Instance_ID'], row['Instance_name'], row['Device_ID'], row['Parent_instance'], row['Manager_name'], row['Manager_mail'], row['Manager_tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['Primary_ip4'], row['Primary_ip6'], row['created'], row['Device_ID'], row['Parent_instance'], row['Manager_name'], row['Manager_mail'], row['Manager_tenant'], row['Type'], row['Status'], row['Level_importance'], row['Monitored'], row['Primary_ip6'], row['created'])
