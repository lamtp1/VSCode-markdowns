    ...
    for id in rp3:
        ...
        if id['neighbor_interface'] != None:
            ...
            for index, row in df.iterrows():
                sql = "INSERT INTO Cloud_HA (Sw_Drc_Object_ID, Sw_ID, Interface_name, Neighbor_MAC, Neighbor_device_ID, Neighbor_instance_ID) VALUES (%s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE  Sw_Drc_Object_ID=%s, Sw_ID=%s, Interface_name=%s, Neighbor_MAC=%s, Neighbor_device_ID=%s, Neighbor_instance_ID=%s"
                val = (row['Sw_Drc_Object_ID'], row['Sw_ID'], row['Interface_name'], row['Neighbor_MAC'], row['Neighbor_device_ID'], row['Neighbor_instance_ID'], row['Sw_Drc_Object_ID'], row['Sw_ID'], row['Interface_name'], row['Neighbor_MAC'], row['Neighbor_device_ID'], row['Neighbor_instance_ID'])
